...
export function CoordStack({ a, b, aColor, bColor, onAChange, onBChange }: CoordStackProps) {
  return (
    <div className="flex flex-col gap-0.5 w-8 md:w-10">
      <Input
        ...
        className={`text-center p-0.5 md:p-1 font-black border-2 text-[10px] md:text-xs h-6 md:h-7 shadow-none ${fieldColorClasses(aColor)}`}
      />
      <Input
        type="text"
        inputMode="numeric"
        ...
        className={`text-center p-0.5 md:p-1 font-black border-2 text-[10px] md:text-xs h-6 md:h-7 shadow-none ${fieldColorClasses(bColor)}`}
      />
    </div>
  )
}
