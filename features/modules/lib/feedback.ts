  ...
  switch (item.type) {
    ...
    case "koordinat": {
      if (slug === "refleksi") {
        ...
        if (isHint) {
          return `• Ingat kembali sifat refleksi terhadap ${axis}: ${rule?.hint ?? "koordinat yang tegak lurus sumbu berubah tanda"}. Terapkan pada titik ${item.label}.`
        }
        return `• Titik ${item.label} direfleksikan terhadap ${axis}: ${rule?.rule ?? "koordinat yang sesuai berubah tanda"}, sehingga bayangannya (${item.answer.x}, ${item.answer.y}).`
      }
      if (isHint) {
        return `• Terapkan rumus translasi: titik (x, y) digeser sejauh (a, b) menjadi (x + a, y + b). Jumlahkan koordinat ${item.label} dengan komponen translasi ${item.bayangan ?? "yang diberikan"}.`
      }
      return `• Titik ${item.label} ditranslasikan sejauh ${item.bayangan ?? ""}: x' = x + a dan y' = y + b, sehingga bayangannya (${item.answer.x}, ${item.answer.y}).`
    }
    case "pilihan_ganda": {
      if (isHint) {
...
export function buildDeterministicFeedback(
  ...
  for (const item of context.items) {
    ...
    lines.push(buildItemFeedback(item, context.answers, context.attempt, context.module, context.tab))
  }
  return lines.join("\n")
}
