...
export function validateRedirect(url: string | null): string {
  if (!url) return "/prasyarat"

  // Decode URI components to prevent encoded attacks
  const decoded = decodeURIComponent(url)

  // Must start with /
  if (!decoded.startsWith("/")) return "/prasyarat"

  // Must not contain protocol (http://, https://, //)
  if (/^https?:\/\//.test(decoded) || decoded.startsWith("//")) return "/prasyarat"

  // Must not contain path traversal
  if (decoded.includes("..")) return "/prasyarat"

  // Must not exceed max length
  if (decoded.length > MAX_REDIRECT_LENGTH) return "/prasyarat"

  // Must not contain spaces or special characters that could indicate injection
  if (/\s/.test(decoded) || /[<>"'`]/.test(decoded)) return "/prasyarat"
