...
Use **RetroUI Button** (`@/components/retroui/Button`) everywhere. It is a shadcn-style wrapper around `@base-ui/react/button` with cva variants.

```jsx
import { Button } from "@/components/retroui/Button"

// Primary CTA (gold bg, shadow, press effect)
...
// Outline / secondary (white bg, thick border)
<Button variant="default"
  className="bg-white border-4 border-black !rounded-none
    text-xs font-bold uppercase hover:bg-secondary-container">
  LABEL
</Button>

// Tiny ghost icon (e.g. password toggle)
...
- Always `!rounded-none` (RetroUI defaults may add rounding)
- Always `font-extrabold` or `font-black`
- Always `uppercase`
- Add `neubrutal-shadow hover-shift active-shift` for primary CTAs
- Use custom className overrides when RetroUI variant defaults don't match the desired styling
...
```jsx
<input
  className="w-full border-2 border-black p-2.5 text-xs font-bold
...
```jsx
<div
  className="bg-[#ffd1ce] border-4 border-black p-4 sm:p-5 mb-6
  neubrutalism-shadow flex flex-col sm:flex-row justify-between
  items-center gap-4"
>
  <h2 className="text-xl font-black uppercase text-[#ae2f34]">Section Title</h2>
</div>
```
...
```jsx
<label
  className="flex items-start gap-3 p-2.5 border-2 border-black rounded cursor-pointer
  [&:has(input:checked)]:bg-[#93f59c]/20 [&:has(input:checked)]:border-[#006e29]"
>
  <input type="checkbox" className="accent-black mt-0.5" />
  <span className="text-xs font-bold">Step text</span>
</label>
```
