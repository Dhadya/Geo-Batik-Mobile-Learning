...
- Route protection: unauthenticated users cannot access `/apersepsi`, `/translasi/*`, `/refleksi/*`, `/lab-batik`
...
**Acceptance Criteria:**
- [ ] Quiz route `/kuis/translasi` exists
...
**Acceptance Criteria:**
- [ ] Quiz route `/kuis/refleksi` exists
...
│   │   └── layout.tsx
│   ├── (learning)/
│   │   ├── translasi/[subtopic]/page.tsx
│   │   ├── refleksi/[subtopic]/page.tsx
│   │   ├── lab-batik/page.tsx
│   │   └── layout.tsx
│   ├── (quiz)/
│   │   ├── kuis/translasi/page.tsx
│   │   ├── kuis/refleksi/page.tsx
│   │   └── layout.tsx
│   ├── api/
│   │   ├── quiz/route.ts
│   │   ├── progress/route.ts
...
│   └── page.tsx
├── components/
│   ├── ui/                    # Shared neubrutalism components
│   │   ├── Button.tsx
│   │   ├── Card.tsx
│   │   ├── Input.tsx
│   │   ├── Tabs.tsx
...
│   │   ├── LabBatikCanvas.tsx
│   │   └── BatikMotifs.ts       # Canvas drawing functions
│   ├── geogebra/
│   │   └── GeoGebraEmbed.tsx
│   ├── learning/
│   │   ├── TranslationLayout.tsx
│   │   ├── ReflectionLayout.tsx
│   │   ├── InquirySteps.tsx
│   │   ├── Observations.tsx
...
├── types/
│   └── index.ts
└── index.css                   # Design tokens, neubrutalism utilities
```
...
> Full reference: [StyleGuide.md](./StyleGuide.md)

### 7.1 Design Language

**Neubrutalism** — thick black borders, hard solid shadows, square elements, uppercase bold typography, high-contrast colors.

### 7.2 Color Palette

| Role | Color | Hex |
|------|-------|-----|
| Primary | Sogan Kuning (olive gold) | `#705d00` |
| Secondary | Pesisir Hijau (forest green) | `#006e29` |
| Tertiary | Merah Gentongan (deep red) | `#ae2f34` |
| Background | Warm off-white | `#fff8ef` |
| Primary Container | Bright gold | `#ffd93d` |
| Secondary Container | Lime green | `#93f59c` |
| Tertiary Container | Coral pink | `#ffd1ce` |
| Text | Dark near-black | `#1f1b12` |
...
| Element | Pattern |
|---------|---------|
| Buttons | Square, `border-2 border-black`, `font-extrabold uppercase`, press effect |
| Cards | `bg-white border-4 border-black neubrutalism-shadow` |
| Inputs | `border-2 border-black p-2.5 text-xs font-bold rounded` |
| Tabs | `border-4 border-black`, active tab raised with `translate-y-[-2px]` |
| Shadows | Hard offset: `8px 8px 0px 0px rgba(0,0,0,1)` |
...
interface AppStore {
  ...
  updateProgress: (module: string, subtopic: string, update: Partial<SubtopicProgress>) => void;
...
```
Welcome → Login (Clerk) → Dashboard (Apersepsi)
  │
  ├──→ Prerequisite Material → Self-check → Dashboard
  │
  ├──→ Translation Module
  │     ├──→ Translasi Titik → Cultural Context → Interactive Canvas →
  │     │    Inquiry Steps → Observations → Conclusion → Comprehension Check
  │     ├──→ Translasi Garis → [same flow]
  │     └──→ Translasi Bidang → [same flow]
  │     └──→ Translation Quiz → Results → Feedback → Dashboard
  │
  ├──→ Reflection Module
  │     ├──→ Refleksi Sumbu X → [same flow as Translation]
  │     ├──→ Refleksi Sumbu Y → [same flow]
  │     ├──→ ... (5 more subtopics)
  │     └──→ Reflection Quiz → Results → Feedback → Dashboard
  │
  └──→ Lab Batik (Creative Sandbox)
        → Place Stamps → Transform → Save → Share
```
